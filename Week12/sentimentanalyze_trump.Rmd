...
#  Inset the Speech of Donald Trumps:
...
# Splitting each page into lines, transform it to regular colums and remove needless spaces
# Siderne splittes op, og jeg konverterer Trumps tale indtil et dataframe. Først bliver den en kolonne, men jeg vil have den splittet op, sådan at hver linje er i sin egen række. Derfor anvender jeg "mutate", som laver en ny kolonne, hvor alle linjer kan indgå. \n deler linjerne op og alle linjer får derved deres egen række. 
...
#  Spliting each word in a new columns and in its own row
...
# Remove stop words with a stopwordlsist:
# Jeg anvender "`tidyr::anti_join()`:" som stopwordslist 
...
# Counting the words after the stoplist
...
# Taking the top 100 most frequent words
...
# Jeg gør min wordcloud mere læselig vedhjælp af ggplots funktioner. Derved farves de ord, som anvendes mest.
...
# afinn
# Jeg anvender ordlisten, som er filtret med stopwordslisten. Jeg bruger "inner_join" for at matche ordene med afinn leksikonet.
...
# Investigate the words on ranking "2":
# Ordene som har værdien 2, og udvælgelsen af kun unikke ord med "distinct"
...
# Show the words there are excluded
...
# Jeg laver mit diagram over ordene i hver kategori
...
```{r nrc-vote filtering, include=FALSE}
# her filtrer man bestemte ord væk

remove_vote <- get_sentiments(lexicon = "nrc") %>% 
  filter(word!="vote")

remove_vote %>% 
  filter(grepl("^vo",word))
```


## Final Question
Ved at lave en sentiment-analyse af Donald Trumps tale i ”afinn” og “NRC” får jeg en varieret tilgang til at analysere talens positiv/negativ ladning. Denne fortolknings udgangspunkt er primært “NCR” analysen. Dog vil jeg blot notere, at begge sentiment-analyser viser, at talen er overvejende positiv ladet, men at talen også indeholder en del negative ladninger og følelser i dens ord og forståelse. For at underbygge denne påstand  kan jeg anvende min udregning af medianen ud fra “afinn” analysen. Her har ordene fået en værdi fra “-5 til 5”. Beregningen viser, at medianen er 1, dermed er talen langt fra kun positiv, da den er tæt på grænsen til minus, som er negativeladninger.

Endvidere er det vigtigt at være opmærksom på, at man ikke blot kan kategorisere ét ord på en bestemt måde, som er tydeligt i min NCR analyse. Her bliver ordet “vote” registreret ved i alt 8 følelser ud af 10. De to følelses-kategorier, hvor "vote" ikke er registreret, er "fear" og "disgust". Dette er interessant i forhold til talens tendens og dens ophavssituation. Donald Trump afholder denne tale efter han er blevet indsat som præsident, og derfor har ordet “vote” en central rolle i forhold til, at han er blevet valgt. Dermed er det folket, som netop har stemt/valgt ham, som han sætter i fokus. Det er interessant at se, at "vote" både får fuld score i “surprise” og “anger”. Hvis jeg ser på “vote” i forhold til "surprise", kan dette referere til, at Trump er overrasket over de stemmer, som han har indhentet. Dette kan være fra etniciteter, som han ikke regnede med ville stemme på ham.  Jeg kan ud fra en læsning af talen bekræfte denne antagelse. Når  "vote" får et højt match ved "anger", kan det skildre Trumps tilhørsforhold til dem, som ikke stemte på ham. Sentiment-analysen giver et overblik over Trumps politiske holdninger og reaktioner på at blive præsident. Særligt NRC giver et overblik over, hvordan Trumps holdning til bestemte emner er. Ulempen ved en sentiment-analyse er, at den ikke viser konteksten, hvor ordene optræder. Fx bliver “immediately” matchet med “anticipation”, dette kræver, at man selv skal forstå teksten og konteksten for at kunne forklare, hvorfor det sker. I den sammenhæng skal det forstås som, at Trump har forventninger og agendaer, som skal ske med det samme.
